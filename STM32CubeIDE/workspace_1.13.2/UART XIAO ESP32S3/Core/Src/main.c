...
/* USER CODE BEGIN PV */
static char uart6_tx_buffer[128];
static uint32_t uart6_sequence = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  {
      /*
       * Replace distance_mm with actual VL53L9CX data later.
       */
      uint16_t distance_mm = 1234;

      int length = snprintf(
          uart6_tx_buffer,
          sizeof(uart6_tx_buffer),
          "{\"sequence\":%lu,\"distance_mm\":%u}\r\n",
          (unsigned long)uart6_sequence,
          (unsigned int)distance_mm
      );

      if ((length > 0) &&
          (length < (int)sizeof(uart6_tx_buffer)))
      {
